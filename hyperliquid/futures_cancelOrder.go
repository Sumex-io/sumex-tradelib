...
func (s *futures_cancelOrder) Do(ctx context.Context, opts ...utils.RequestOption) (res []entity.PlaceOrder, err error) {
	if s.symbol == nil || strings.TrimSpace(*s.symbol) == "" {
		return nil, fmt.Errorf("hyperliquid futures cancelOrder: symbol is required")
	}
	if s.orderID == nil || strings.TrimSpace(*s.orderID) == "" {
		return nil, fmt.Errorf("hyperliquid futures cancelOrder: orderID is required")
	}
	...
	if err != nil || oid <= 0 {
		return nil, fmt.Errorf("hyperliquid futures cancelOrder: invalid orderID %q", *s.orderID)
	}
