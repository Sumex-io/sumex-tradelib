...
// ===============ACTION BUILDERS=================
//
// Declared here so the client's factory surface above compiles and is reviewable as one list. The
// per-action pass moves each struct into its own <scope>_<action>.go alongside its option setters
// and its Do method, exactly as hyperliquid/ lays them out; nothing else about them changes.
